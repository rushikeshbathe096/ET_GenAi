...
import { ShieldCheck, Mail, Lock, UserPlus, Loader2, RefreshCcw } from "lucide-react"
    ...
    <div className={`min-h-screen flex flex-col items-center justify-center ${theme.pageBg} ${theme.textMain} font-sans transition-colors duration-300 px-4`}>
      ...
      <div className="z-10 w-full max-w-md py-8">
        ...
        >
          ...
          <p className={`mt-2 text-sm ${theme.textMuted}`}>Create an analyst account.</p>
        </motion.div>
        ...
        >
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <RefreshCcw className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 ${theme.textSoft}`} />
                <input 
                  type="password" 
            ...
            <motion.button
              ...
              whileHover={!(isLoading || successMsg) ? { y: -2, scale: 1.01 } : {}}
              whileTap={!(isLoading || successMsg) ? { scale: 0.98 } : {}}
              className="w-full mt-4 flex items-center justify-center gap-2 rounded-xl bg-cyan-600 px-6 py-3.5 text-sm font-semibold text-white shadow-[0_0_20px_#0891b266] transition hover:bg-cyan-500 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
          ...
          <div className={`mt-8 pt-6 border-t ${isDarkMode ? "border-white/10" : "border-slate-200"} text-center`}>
            <p className={`text-sm ${theme.textMuted}`}>
              ...
              <Link href="/signin" className={`font-semibold hover:underline ${theme.accentText}`}>
                Sign In
              </Link>
            </p>
          </div>
